...
export default function MainGoalItem({ id, age, categories, text, likes, comments }) {
  return (
    <Link href={`/goal`}>
      <li>
        <div className="item">
          <div className="info">
            익명의 {id}님 | {parseInt(age / 10) * 10}대
          </div>
          <div className="categories">
            <ul>
              ...
              {categories.map((item) => {
                return <li key={item}>#{item.length > 3 ? item.slice(0, 3) + ".." : item}</li>;
              })}
            </ul>
          </div>
          <div className="text">{text.length > 95 ? text.slice(0, 95) + " ..." : text}</div>
          <div className="Icons">
            <div className="likes">
